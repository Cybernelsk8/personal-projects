...
<template>
    <section class="px-7">
        ...
        <div class="md:flex md:items-center md:justify-between">
            <div class="text-slate-600  flex items-center px-2 py-1.5">
                <span>Mostrar</span>
                <select v-model="rowsPerPage" @change="resetPage"class="text-center bg-slate-900 w-full focus:outline-none ring-0">
                    <option>5</option>
                    <option>10</option>
                    <option>25</option>
                    <option>50</option>
            ...
            <div class="flex items-center gap-1">
                ...
                <Input icon="fas fa-search" placeholder="Search...." type="search" v-model="search" />

                <div class="flex gap-1">
                    <!-- FILTER ADVANCE -->
                    <div class="relative" ref="target">
                        <Tool-Tip message="Filtro avanzado" class="-mt-7 text-slate-600" v-if="props.filterAdvance">
                            <Icon @click="openFilterOptions = !openFilterOptions" icon="fas fa-filter" class="icon-button p-3" :class="filters.length > 0 && filters[0].value != '' ? 'bg-green-500' : 'bg-gray-400'" />
                        </Tool-Tip>
                    ...
                    <!-- END FILTER ADVANCE -->
                    <Tool-Tip message="Descargar" class="-mt-7 text-gray-500">
                        <Icon v-if="props.export && data.length > 0" @click="exportData" :loading="loadingExportData" icon="fas fa-file-excel" class="icon-button p-3 bg-green-500" />
                    </Tool-Tip>
                </div>
            </div>
        </div>
        ...
        <!-- TABLE -->
        <Tabla :color="props.color">
            <template #thead>
                <tr>
                    <th v-if="props.multiSelect">
                        <input type="checkbox" ref="selectAll" @change="selectAllItems">
                    </th>
                    <th v-for="(head, index) in props.headers" :key="index" @click="sort(head.key, head.type)" scope="col" class="py-3.5 text-sm font-normal cursor-pointer select-none" :width="head.width" :align="head.align ?? 'left'" :hidden="head.hidden">
            ...
            <template #tbody class="bg-white divide-y divide-gray-200">
                <slot name="tbody" :items="paginatedData">
                    <tr v-for="item in paginatedData" :key="item.id" class="hover:bg-slate-800 select-none">
                        <td v-if="props.multiSelect" align="center"> 
                            <input type="checkbox" @change="selectdAll" v-model="selectItems" :value="item"> 
                        </td>
                        <td v-for="(head, index) in props.headers" :align="head.align ?? 'left'" :width="head.width" :key="index" :hidden="head.hidden">
                ...
                <tr v-if="props.loading">
                    <td align="center" :colspan="props.headers.length" class="px-10">
                        <Loading-Bar class="h-1 bg-violet-300" />
