<script setup>
    ...
    const searching = computed(() => {
        return resultSearch.value.filter(item => item.name.toLowerCase().match(search.value.toLowerCase())).slice(0,5)
    })
...
<template>
    <div class="relative border-2 rounded-lg">
            ...
            <ul class="flex flex-wrap gap-2">
                <li v-for="(select,index) in selected" class=" whitespace-break-spaces">
                    <div class="flex items-center relative">
                        <small class="bg-blue-400 text-xs rounded-full whitespace-nowrap tracking-tight px-2 py-2 text-blue-100">{{ select.name ?? '' }}</small>
        ...
        <Transition>
            <ul v-if="open" class="w-full bg-white border-2 rounded-lg mt-1 py-4 overflow-auto absolute z-10">
